...
def getHistoricalBarData(barType,connection,ibContract,timeFrame,chartTime):
    if (barType == Config.entryTradeType[0]) or (barType == Config.entryTradeType[2]):  # Custom or ASK + 1/2
...
def _is_extended_outside_rth(outsideRth: bool):
    """
    Returns (is_extended, session). is_extended is True when the current session (US/Eastern)
    is PREMARKET or AFTERHOURS, so we use stop-limit (STP LMT) for entry orders.
    Also True when the user set outsideRth=True (user intends OTH). This avoids sending
    RTH-style orders when the exchange is closed (Error 201).
    """
    session = _get_current_session()
    # Use STP LMT when we're actually in extended hours (exchange is closed to RTH orders)
    # or when the user explicitly selected outside hours (OTH)
    is_extended = (session in ('PREMARKET', 'AFTERHOURS')) or outsideRth
...
# Premarket/postmarket only: stop-limit order for stop loss uses limit = stop ± (this multiplier × stop_size)
# Long (close with SELL): limit = stop - 3×stop_size; Short (close with BUY): limit = stop + 3×stop_size
EXTENDED_HOURS_SL_LIMIT_OFFSET_MULT = 3
...
async def rb_and_rbb(connection, symbol,timeFrame,profit,stopLoss,risk,tif,barType,buySellType,atrPercentage,quantity,pullBackNo,slValue ,breakEven,outsideRth,entry_points):
    ...
    while True:
        ...
        if Config.historicalData.get(key) == None:
            # Get the most recent bar data (not waiting for specific time)
            logging.info("Getting most recent bar data for RBB: symbol=%s, timeFrame=%s", symbol, timeFrame)
            histData = connection.rbb_entry_historical_data(ibContract, timeFrame, chartTime)
            if (histData is None or len(histData) == 0):
                logging.info("RBB Chart Data not available for %s contract, trying fallback...", ibContract)
                ...
                if outsideRth:
                    ...
                    # Regular hours: Try to get market price as fallback if historical data fails
                    # Add a retry counter to prevent infinite loop
                    if not hasattr(rb_and_rbb, '_retry_count'):
                        rb_and_rbb._retry_count = {}
                    if key not in rb_and_rbb._retry_count:
                        rb_and_rbb._retry_count[key] = 0
                    
                    rb_and_rbb._retry_count[key] += 1
                    max_retries = 30  # Try for 30 seconds
                    
                    if rb_and_rbb._retry_count[key] >= max_retries:
                        logging.warning("RBB: Historical data not available after %d retries. Attempting to proceed with market price fallback...", max_retries)
                        # Try to get current market price as fallback
                        try:
                            ticker = connection.ib.reqMktData(ibContract, '', False, False)
                            connection.ib.waitOnUpdate(timeout=2)
                            if ticker and hasattr(ticker, 'last') and ticker.last:
                                current_price = ticker.last
                                logging.info("RBB: Using current market price %s as fallback for historical data", current_price)
                                histData = {
                                    'close': current_price,
                                    'open': current_price,
                                    'high': current_price,
                                ...
                                logging.error("RBB: Could not get market price, will retry")
                                await asyncio.sleep(1)
                                continue
                        except Exception as e:
                            logging.error("RBB: Error getting market price fallback: %s", e)
                            await asyncio.sleep(1)
                            continue
                    else:
                        await asyncio.sleep(1)
                ...
                if hasattr(rb_and_rbb, '_retry_count') and key in rb_and_rbb._retry_count:
                    rb_and_rbb._retry_count[key] = 0

        # if barType == Config.entryTradeType[Config.CONDITIONAL_ORDER_INDEX]:
        ...
        if is_extended:
            ...
            if barType == Config.entryTradeType[Config.RBB_INDEX]:  # RBB - place only entry order, TP/SL after fill
                ...
                try:
                    ...
                    entry_response = connection.placeTrade(contract=ibContract, order=entry_order, outsideRth=outsideRth, trade_type=barType)
                    # Get the actual entry order ID from response (may have been changed if duplicate was detected)
                    actual_entry_order_id = entry_response.order.orderId
                    logging.info("RB RTH: Entry order placed successfully - orderId=%s", actual_entry_order_id)
                    
                    # Update bracket orders to use actual entry order ID (in case it was changed)
                    if actual_entry_order_id != parent_order_id:
                        logging.info("RB RTH: Entry order ID changed from %s to %s, updating bracket orders", 
                                   parent_order_id, actual_entry_order_id)
                        tp_order.parentId = actual_entry_order_id
                        sl_order.parentId = actual_entry_order_id
...
async def manual_limit_order_ask_bid_half(connection, symbol, timeFrame, profit, stopLoss, risk, tif, barType, buySellType,
    ...
    ask, bid = await _get_ask_bid(connection, contract, wait_sec=0.5)
    # Fallback when ask/bid unavailable (e.g. premarket, illiquid): use latest bar close as reference
    if ask is None and bid is None:
        try:
            hist_bar = _get_latest_hist_bar(connection, contract, timeFrame)
            if hist_bar and hist_bar.get('close') is not None:
                close_price = float(hist_bar['close'])
                if not math.isnan(close_price):
                    ask, bid = close_price, close_price
                    logging.info("ASK + 1/2 / BID - 1/2: No tick data for %s, using latest bar close=%s as reference", symbol, close_price)
        except Exception as e:
            logging.debug("ASK + 1/2 / BID - 1/2 fallback to hist bar: %s", e)
    if ask is None and bid is None:
        logging.error("ASK + 1/2 / BID - 1/2: Could not get market data (ask/bid) or latest bar for %s", symbol)
        return
    if barType == 'ASK + 1/2':
    ...
    try:
        ...
        if is_lod_hod:
            ...
            try:
                ...
                entry_response = connection.placeTrade(contract=contract, order=entry_order, outsideRth=outsideRth, trade_type=barType)
        ...
        if is_extended:
            ...
            # Update bracket orders to use actual entry order ID (in case it was changed)
            if actual_entry_order_id != parent_order_id:
                logging.info("RTH Limit Order: Entry order ID changed from %s to %s, updating bracket orders", 
                           parent_order_id, actual_entry_order_id)
                tp_order.parentId = actual_entry_order_id
                sl_order.parentId = actual_entry_order_id
            StatusUpdate(entry_response, 'Entry', contract, 'LMT', buySellType, qty, histData, entry_price, symbol,
...
async def manual_stop_order(connection, symbol, timeFrame, profit, stopLoss, risk, tif, barType, buySellType,
    ...
    try:
        ...
        if stopLoss in Config.atrStopLossMap:
            ...
            # HOD/LOD can yield stop_size=0 when entry equals session high/low; use minimum so order can be placed
            if stopLoss in (Config.stopLoss[3], Config.stopLoss[4]) and (stop_size == 0 or stop_size < 0.01):
                stop_size = 0.01
                logging.warning("Manual stop order %s: HOD/LOD stop_size was 0 or very small, using minimum 0.01", symbol)
            if stop_size == 0 or math.isnan(stop_size):
        ...
        # For LOD/HOD:
        # - RTH: STP entry is fine
        # - Extended hours (premarket/after-hours): MUST use STP LMT (stop orders may not execute reliably)
        if is_lod_hod:
            # entry_stop_price is already calculated in the is_lod_hod block above
            entry_order_type = "STP LMT" if is_extended else "STP"
            entry_limit_price = None
            if is_extended:
                # Use a conservative limit around stop price so the order can fill in extended hours
                entry_limit_offset = round(max(0.01, float(stop_size) * 0.5), Config.roundVal)
                if buySellType == 'BUY':
                    entry_limit_price = round(float(entry_stop_price) + entry_limit_offset, Config.roundVal)
                else:
                    entry_limit_price = round(float(entry_stop_price) - entry_limit_offset, Config.roundVal)
            ...
            entry_order = Order(
                ...
                totalQuantity=qty,
                auxPrice=entry_stop_price,
                lmtPrice=entry_limit_price if is_extended else None,
                tif=tif,
                transmit=True  # Transmit immediately - TP/SL will be sent after fill
            )
            ...
            try:
                if is_extended:
                    logging.info(
                        "Placing LOD/HOD %s order: orderId=%s, action=%s, qty=%s, stop=%s, limit=%s, tif=%s, outsideRth=%s",
                        entry_order_type, parent_order_id, buySellType, qty, entry_stop_price, entry_limit_price, tif, outsideRth
                    )
                else:
                    logging.info(
                        "Placing LOD/HOD %s order: orderId=%s, action=%s, qty=%s, stop=%s, tif=%s, outsideRth=%s",
                        entry_order_type, parent_order_id, buySellType, qty, entry_stop_price, tif, outsideRth
                    )
                entry_response = connection.placeTrade(contract=contract, order=entry_order, outsideRth=outsideRth, trade_type=barType)
                if entry_response is None:
                    logging.error("placeTrade returned None for LOD/HOD %s order %s", entry_order_type, symbol)
                    return
                logging.info("placeTrade successful: orderId=%s, status=%s",
                             entry_response.order.orderId, entry_response.orderStatus.status)
                StatusUpdate(entry_response, 'Entry', contract, entry_order_type, buySellType, qty, histData, entry_stop_price, symbol,
                             timeFrame, profit, stopLoss, risk, '', tif, barType, buySellType, atrPercentage, slValue,
                             breakEven, outsideRth, False, entry_points)

                # Store stop_size and stop_loss_price in orderStatusData (used by TP/SL placement)
                if int(entry_response.order.orderId) in Config.orderStatusData:
                    Config.orderStatusData[int(entry_response.order.orderId)]['stopSize'] = stop_size
                    Config.orderStatusData[int(entry_response.order.orderId)]['stopLossPrice'] = stop_loss_price
                    logging.info(
                        "Stored stop_size=%s, stop_loss_price=%s in orderStatusData for LOD/HOD order %s",
                        stop_size, stop_loss_price, entry_response.order.orderId
                    )

                logging.info("LOD/HOD: Entry %s order placed. TP and SL will be sent automatically after entry fills.", entry_order_type)
            except Exception as place_error:
                logging.error("Error placing LOD/HOD %s order for %s: %s", entry_order_type, symbol, place_error)
                logging.error("Traceback: %s", traceback.format_exc())
                raise
            return  # Exit early for LOD/HOD
        ...
        use_stop_limit_entry = is_extended or outsideRth  # OTH: all entry orders should be stop-limit (excluding Limit Order trade type)
        if use_stop_limit_entry:
            # OTH / Extended hours (premarket/after-hours) for Custom trade type:
            ...
            logging.info("RTH Stop Order: Entry order placed - orderId=%s, status=%s", 
                         actual_entry_order_id, entry_response.orderStatus.status)
            
            # Update bracket orders to use the actual entry order ID (in case it was changed)
            if actual_entry_order_id != parent_order_id:
                logging.info("RTH Stop Order: Entry order ID changed from %s to %s, updating bracket orders", 
                           parent_order_id, actual_entry_order_id)
                tp_order.parentId = actual_entry_order_id
                sl_order.parentId = actual_entry_order_id
...
def _pbe_has_3_consecutive_bars(complete_bar_data, buy_sell_type):
    """
    Return True if the last 3 completed bars are 3 consecutive green (for BUY/long)
    or 3 consecutive red (for SELL/short). Green = close > open, Red = close < open.
    complete_bar_data is dict keyed 0,1,2,...; last bar (highest index) is current forming,
    so last 3 completed bars are indices len-4, len-3, len-2.
    """
    n = len(complete_bar_data)
    if n < 4:
        return False
    indices = [n - 4, n - 3, n - 2]
    for i in indices:
        bar = complete_bar_data.get(i)
        if not bar or 'open' not in bar or 'close' not in bar:
            return False
        ...
        if buy_sell_type == 'BUY':
            ...
            if green:  # need red (close < open)
                return False
    return True


async def pull_back_PBe1(connection, symbol,timeFrame,profit,stopLoss,risk,tif,barType,buySellType,atrPercentage,quantity,pullBackNo,slValue,breakEven,outsideRth,entry_points, require_3_consecutive_bars=False):
        ...
        while True:
            try:
                ...
                # PBe1 (3): require 3 consecutive green bars (long) or 3 red bars (short) before triggering
                if require_3_consecutive_bars:
                    if len(complete_bar_data) < 4:
                        logging.info("PBe1 (3): need at least 4 bars for 3 completed, have %s; waiting...", len(complete_bar_data))
                        await asyncio.sleep(1)
                        continue
                    if not _pbe_has_3_consecutive_bars(complete_bar_data, buySellType):
                        logging.info("PBe1 (3): 3 consecutive %s bars not yet met; waiting...", "green" if buySellType == 'BUY' else "red")
                        await asyncio.sleep(1)
                        continue
                    logging.info("PBe1 (3): 3 consecutive %s bars condition met; triggering PBe1", "green" if buySellType == 'BUY' else "red")
                ...
                # Entry stop price is simple: bar_high + 0.01 (BUY) or bar_low - 0.01 (SELL) - like regular RBB, same as PBe2
...
async def pull_back_PBe2(connection, symbol,timeFrame,profit,stopLoss,risk,tif,barType,buySellType,atrPercentage,quantity,pullBackNo,slValue ,breakEven,outsideRth, require_3_consecutive_bars=False):
    """
    PBe2 logic (aligned with real PBe1 — same bar and direction, no extra condition):
    - The PBe1 leg in PBe2 is for CALCULATION ONLY — no real PBe1 order is placed.
    - Use same logic as real PBe1: buySellType directly (no pbe1_result pattern check; real PBe1 does not use it).
    - Simulate PBe1 without placing any order (calculate only: entry fill then SL/TP hit):
      ...
      * Stop out is only recognized after at least one new bar has closed since "entry fill"
        so PBe2 does not trigger on the same bar as a real PBe1 entry.
    ...
    while True:
        ...
        # PBe2 logic (aligned with real PBe1):
        # 1. Use same bar and direction as real PBe1: buySellType directly, NO pbe1_result() condition (real PBe1 does not check pattern)
        # 2. Simulate PBe1: entry fill then stop out (LOD for BUY, HOD for SELL)
        # 3. When simulated PBe1 stops out, replay PBe1 (place real PBe2 order)
        
        logging.info("PBe2: Starting PBe1 simulation (same bar + direction as real PBe1, no pattern condition)")
        
        # Step 1: Get bar data - same as real PBe1. Use buySellType directly (no pbe1_result check; real PBe1 does not use it)
        complete_bar_data = connection.pbe1_entry_historical_data(ibContract, timeFrame, chartTime)
        logging.info("PBe2: RecentBar for PBe1 simulation %s", complete_bar_data)
        if(len(complete_bar_data) == 0):
            logging.info("PBe2: last 1 record not found for PBe1 simulation, will try after 1 sec.")
            await asyncio.sleep(1)
            continue

        # PBe2 (3): require 3 consecutive green bars (long) or 3 red bars (short) before triggering
        if require_3_consecutive_bars:
            if len(complete_bar_data) < 4:
                logging.info("PBe2 (3): need at least 4 bars for 3 completed, have %s; waiting...", len(complete_bar_data))
                await asyncio.sleep(1)
                continue
            if not _pbe_has_3_consecutive_bars(complete_bar_data, buySellType):
                logging.info("PBe2 (3): 3 consecutive %s bars not yet met; waiting...", "green" if buySellType == 'BUY' else "red")
                await asyncio.sleep(1)
                continue
            logging.info("PBe2 (3): 3 consecutive %s bars condition met; triggering PBe2", "green" if buySellType == 'BUY' else "red")
        ...
        if len(complete_bar_data) >= 2:
            ...
            last_candel = complete_bar_data[len(complete_bar_data)-2]
            logging.info("PBe2: Using second-to-last bar (index=%s) as most recent closed bar - same as real PBe1", len(complete_bar_data)-2)
        else:
            # Only one bar available, use it (might be the initial bar) - same as PBe1
            last_candel = complete_bar_data[len(complete_bar_data)-1]
            logging.info("PBe2: Only one bar available, using it (index=0) - same as real PBe1")
        
        logging.info("PBe2: last candel for PBe1 simulation %s ", last_candel)
        if (last_candel == None or len(last_candel) == 0):
            logging.info("PBe2: Last Price Not Found for %s contract", ibContract)
            await asyncio.sleep(1)
            continue
        
        # PBe2: Use buySellType directly - SAME AS REAL PBe1 (real PBe1 does NOT use pbe1_result)
        pbe1_tradeType = buySellType
        logging.info("PBe2: Using user-selected trade type %s (no condition check - same as real PBe1)", pbe1_tradeType)
        ...
        # Simulated PBe1 take profit (same as PBe1: entry + stop_size * multiplier for BUY, entry - for SELL)
        _tp_multiplier_map = {Config.takeProfit[0]: 1, Config.takeProfit[1]: 1.5, Config.takeProfit[2]: 2, Config.takeProfit[3]: 2.5}
        if len(Config.takeProfit) > 4:
            _tp_multiplier_map[Config.takeProfit[4]] = 3
        _tp_mult = _tp_multiplier_map.get(profit, 1)
        _tp_offset = pbe1_stop_size * _tp_mult
        if pbe1_tradeType == 'BUY':
            pbe1_tp_price = round(pbe1_entry_price + _tp_offset, Config.roundVal)
        else:
            pbe1_tp_price = round(pbe1_entry_price - _tp_offset, Config.roundVal)
        
        logging.info("PBe2: PBe1 would have: entry_price=%s, stop_loss=%s, tp=%s, stop_size=%s, LOD=%s, HOD=%s",
                     pbe1_entry_price, pbe1_stop_loss_price, pbe1_tp_price, pbe1_stop_size, lod, hod)
        logging.info("PBe2: Monitoring for PBe1 entry fill, then stop out (SL or TP hit); then replay real PBe2 entry+TP/SL")
        ...
        pbe1_entry_filled = False  # Track if PBe1 entry would have been filled (calculation only - no real order)
        pbe1_stop_out_allowed = False  # Only True after at least one new bar has closed since entry fill (so PBe2 does not trigger same bar as real PBe1)
        last_processed_bar_datetime = last_candel.get('date') if last_candel else None  # Track last processed bar to detect new bars
        pbe1_stopped_out = False  # True only when SL hit (not TP); PBe2 real order triggers only on stop out, not on take profit
                ...
                try:
                    ...
                    if current_price is None:
                        ...
                        if latest_bar_data and len(latest_bar_data) > 0:
                            # Use high/low from latest bar to check if entry was hit (pbe1_entry_historical_data returns dict keyed by 0,1,2,...)
                    ...
                    # When entry filled but stop out not yet allowed, still fetch bar data so we can detect new bar close
                    elif pbe1_entry_filled and not pbe1_stop_out_allowed:
                        latest_bar_data = connection.pbe1_entry_historical_data(ibContract, timeFrame, chartTime)
                        if latest_bar_data and len(latest_bar_data) > 0:
                            last_key = max(latest_bar_data.keys())
                            latest_bar = latest_bar_data[last_key]
                    ...
                    # Step 2a: Check if simulated PBe1 entry would have been filled.
                    # When running PBe1 and PBe2 together: treat real PBe1 entry fill as simulated entry fill
                    # (current_price can be "after" the fill — e.g. price bounced — so we'd never see it from price alone)
                    if not pbe1_entry_filled:
                        # Check if real PBe1 entry order (same symbol) is Filled — align PBe2 with real PBe1
                        if len(Config.entryTradeType) > 6:
                            for _oid, _od in Config.orderStatusData.items():
                                if (_od.get('usersymbol') == symbol
                                        and _od.get('barType') == Config.entryTradeType[Config.PBe1_INDEX]  # PBe1
                                        and _od.get('ordType') == 'Entry'
                                        and _od.get('status') == 'Filled'):
                                    pbe1_entry_filled = True
                                    logging.info("PBe2: Real PBe1 entry order is Filled (orderId=%s). Treating simulated PBe1 entry as filled.", _oid)
                                    break
                        # Recalculate LOD/HOD when new bar closes
                        if new_bar_closed:
                            updated_lod, updated_hod = _get_pbe1_lod_hod(connection, ibContract, timeFrame, pbe1_tradeType)
                            if pbe1_tradeType == 'BUY':
                                if updated_lod != lod:
                                    logging.info("PBe2: New bar closed - LOD updated while waiting for entry fill! Old LOD=%s, New LOD=%s.", lod, updated_lod)
                                    lod = updated_lod
                                    pbe1_stop_loss_price = round(lod, Config.roundVal)
                            else:  # SELL
                                if updated_hod != hod:
                                    logging.info("PBe2: New bar closed - HOD updated while waiting for entry fill! Old HOD=%s, New HOD=%s.", hod, updated_hod)
                                    hod = updated_hod
                                    pbe1_stop_loss_price = round(hod, Config.roundVal)
                        # Price-based simulated fill check only if not already filled (e.g. by real PBe1)
                        if not pbe1_entry_filled:
                            if pbe1_tradeType == 'BUY':
                                if current_price >= pbe1_entry_price:
                                    pbe1_entry_filled = True
                                    # pbe1_stop_out_allowed stays False until we see a new bar close (so PBe2 does not trigger same bar as real PBe1)
                                    logging.info("PBe2: PBe1 entry would have been FILLED! (calculation only) Current price=%s >= entry_price=%s. Will wait for at least one bar after this before allowing stop out.",
                                               current_price, pbe1_entry_price)
                                else:
                                    logging.info("PBe2: Waiting for PBe1 entry fill. Current price=%s < entry_price=%s. Waiting...", current_price, pbe1_entry_price)
                                    sleepTime = getSleepTime(timeFrame, outsideRth) or 1
                                    await asyncio.sleep(sleepTime)
                                    continue
                            else:  # SELL
                                if current_price <= pbe1_entry_price:
                    ...
                    if pbe1_entry_filled:
                        # Allow stop out only after at least one new bar has closed since entry fill (PBe2 must not trigger same bar as real PBe1)
                        if new_bar_closed:
                            pbe1_stop_out_allowed = True
                        ...
                        # Get latest bar high and low to check both SL and TP (simulated PBe1: one of them closes the position)
                        # pbe1_entry_historical_data returns dict keyed by 0,1,2,... (not a list)
                        latest_bar_data = connection.pbe1_entry_historical_data(ibContract, timeFrame, chartTime)
                        if latest_bar_data and len(latest_bar_data) > 0:
                            last_key = max(latest_bar_data.keys())
                            _bar = latest_bar_data[last_key]
                            bar_low = round(float(_bar.get('low', _bar.get('close', 0))), Config.roundVal)
                            bar_high = round(float(_bar.get('high', _bar.get('close', 0))), Config.roundVal)
                        else:
                            bar_low = bar_high = round(float(last_candel.get('close', 0)), Config.roundVal)
                        
                        # When running with real PBe1: if real PBe1 stop loss order is Filled, trigger PBe2 immediately (no need to wait for new bar)
                        if len(Config.entryTradeType) > 6:
                            for _oid, _od in Config.orderStatusData.items():
                                if (_od.get('usersymbol') == symbol
                                        and _od.get('barType') == Config.entryTradeType[Config.PBe1_INDEX]  # PBe1
                                        and _od.get('ordType') == 'StopLoss'
                                        and _od.get('status') == 'Filled'):
                                    pbe1_stopped_out = True
                                    logging.info("PBe2: Real PBe1 stop loss order is Filled (orderId=%s). Triggering PBe2 replay.", _oid)
                                    break
                            if pbe1_stopped_out:
                                break
                        # Stop out = price hit stop loss OR take profit (same as real PBe1) - only after at least one bar since entry fill
                        if not pbe1_stop_out_allowed:
                            logging.info("PBe2: PBe1 entry filled (calculation only). Waiting for at least one new bar to close before allowing stop out check...")
                            sleepTime = getSleepTime(timeFrame, outsideRth) or 1
                            await asyncio.sleep(sleepTime)
                            continue
                        if pbe1_tradeType == 'BUY':
                            sl_hit = bar_low <= pbe1_stop_loss_price
                            tp_hit = bar_high >= pbe1_tp_price
                            if sl_hit:
                                pbe1_stopped_out = True
                                logging.info("PBe2: PBe1 stopped out (SL hit)! bar_low=%s, bar_high=%s, SL=%s, TP=%s. Proceeding to replay PBe2...",
                                             bar_low, bar_high, pbe1_stop_loss_price, pbe1_tp_price)
                                break
                            if tp_hit:
                                logging.info("PBe2: PBe1 would have closed at TP (not stopped out). bar_high=%s >= TP=%s. Resuming wait for new PBe1 condition.",
                                             bar_high, pbe1_tp_price)
                                break
                            logging.info("PBe2: PBe1 entry filled. bar_low=%s > SL=%s and bar_high=%s < TP=%s. Waiting for SL or TP...",
                                         bar_low, pbe1_stop_loss_price, bar_high, pbe1_tp_price)
                        else:  # SELL
                            sl_hit = bar_high >= pbe1_stop_loss_price
                            tp_hit = bar_low <= pbe1_tp_price
                            if sl_hit:
                                pbe1_stopped_out = True
                                logging.info("PBe2: PBe1 stopped out (SL hit)! bar_high=%s, bar_low=%s, SL=%s, TP=%s. Proceeding to replay PBe2...",
                                             bar_high, bar_low, pbe1_stop_loss_price, pbe1_tp_price)
                                break
                            if tp_hit:
                                logging.info("PBe2: PBe1 would have closed at TP (not stopped out). bar_low=%s <= TP=%s. Resuming wait for new PBe1 condition.",
                                             bar_low, pbe1_tp_price)
                                break
                            logging.info("PBe2: PBe1 entry filled. bar_high=%s < SL=%s and bar_low=%s > TP=%s. Waiting for SL or TP...",
                                         bar_high, pbe1_stop_loss_price, bar_low, pbe1_tp_price)
                        sleepTime = getSleepTime(timeFrame, outsideRth) or 1
                        await asyncio.sleep(sleepTime)
                        continue
                except Exception as e:
        ...
        # Only place real PBe2 order when simulated PBe1 was STOPPED OUT (SL hit). If TP was hit, resume waiting for new PBe1 condition.
        if not pbe1_stopped_out:
            logging.info("PBe2: Simulated PBe1 closed at TP (not stopped out). Resuming monitoring for new PBe1 condition.")
            continue
        ...
        # Calculate quantity: use same risk as PBe1 when real PBe1 stopped out (match UI risk amount)
        risk_amount = _to_float(risk, 0)
        pbe1_stop_size = None
        pbe1_quantity = None
        if len(Config.entryTradeType) > 6:
            for _oid, _od in Config.orderStatusData.items():
                if (_od.get('usersymbol') == symbol
                        ...
                        and _od.get('ordType') == 'Entry'
                        and _od.get('status') == 'Filled'):
                    pbe1_stop_size = _od.get('stopSize') or _od.get('calculated_stop_size')
                    pbe1_quantity = _od.get('totalQuantity') or _od.get('quantity')
                    if pbe1_quantity is not None and hasattr(pbe1_quantity, '__float__'):
                        pbe1_quantity = int(round(float(pbe1_quantity), 0))
                    elif isinstance(pbe1_quantity, (int, float)):
                        pbe1_quantity = int(round(float(pbe1_quantity), 0))
                    if pbe1_stop_size is not None:
                        pbe1_stop_size = float(pbe1_stop_size)
                    logging.info(f"PBe2: Found real PBe1 entry (orderId={_oid}): stop_size={pbe1_stop_size}, quantity={pbe1_quantity}")
                    break
        
        if (pbe1_quantity is not None and pbe1_quantity > 0):
            # Use same share size as PBe1 when real PBe1 stopped out
            quantity = pbe1_quantity
            logging.info(f"PBe2 quantity (same as PBe1 share size): quantity={quantity}")
        elif risk_amount <= 0:
        ...
        # Check for existing active PBe2 entry order to avoid duplicate orders (e.g. when pull_back_PBe2 is triggered twice)
        existing_pbe2_entry = None
        for _oid, _od in Config.orderStatusData.items():
            if (_od.get('usersymbol') == symbol
                    and _od.get('barType') == barType
                    and _od.get('ordType') == 'Entry'
                    and _od.get('status') in ('PendingSubmit', 'PreSubmitted', 'Submitted')):
                existing_pbe2_entry = (_oid, _od)
                logging.info("PBe2 REPLAY MODE: Found existing active PBe2 entry orderId=%s, status=%s - skipping duplicate placement", _oid, _od.get('status'))
                break
        
        if existing_pbe2_entry is None:
...
async def _update_pbe_stop_size_and_quantity(connection, order_id, is_pbe1=True):
    ...
    try:
        ...
        # Recalculate quantity based on new stop_size (PBe1 only - PBe2 quantity stays fixed to match PBe1 risk)
        old_quantity = old_order.get('totalQuantity', old_order.get('quantity', 0))
        if is_pbe1:
            risk_amount = _to_float(old_order.get('risk', 0), 0)
            if risk_amount <= 0:
                logging.warning(f"PBe1: Invalid risk amount ({risk_amount}), skipping quantity update")
                return
            ...
            # PBe2: Keep quantity fixed - it was set at placement to match PBe1 risk
            # (entry order quantity is never modified by pbe2_loop_run)
        ...
        old_order['totalQuantity'] = new_quantity
        ...
        # If entry is filled, update TP/SL orders with new quantity (PBe2: quantity unchanged, only stop_size/price)
        if old_order['status'] == 'Filled':
        ...
        try:
            ...
            if (old_order['status'] != 'Filled' and old_order['status'] != 'Cancelled' and old_order['status'] != 'Inactive'):
                ...
                if is_extended:
                    ...
                    qty = old_order.get('totalQuantity', old_order.get('quantity', order.totalQuantity))
                    new_order = Order(orderType="STP LMT", action=order.action, totalQuantity=qty, 
                                    tif='DAY', auxPrice=aux_price, lmtPrice=limit_price)
                else:
                    # RTH: Update only entry order (TP/SL are sent separately after fill)
                    logging.info(f"PBe1 RTH Update: STP, Stop={aux_price} (bar_high/low ± 0.01)")
                    qty = old_order.get('totalQuantity', old_order.get('quantity', order.totalQuantity))
                    new_order = Order(orderType="STP", action=order.action, totalQuantity=qty,  
                                    tif='DAY', auxPrice=aux_price)
...
def _get_current_session():
    """Return current US equity session (PREMARKET, RTH, AFTERHOURS, OVERNIGHT) using US/Eastern time.
    US market hours: RTH 09:30-16:00 ET, premarket 04:00-09:30 ET, after-hours 16:00-20:00 ET."""
    try:
...
def _get_lod_hod_for_stop_loss(connection, contract, timeFrame):
    ...
    LOD = minimum price in the session; HOD = maximum price in the session.
    
    - Premarket: uses premarket bars only (04:00 to 09:30).
    - RTH: uses RTH bars from 09:30 to current time.
    - Post-market (after hours): does not recalculate; uses RTH LOD/HOD only (09:30 to 16:00).
    ...
    # Post-market: use RTH LOD/HOD only (no recalculation with after-hours data)
    if session == 'AFTERHOURS':
        logging.info("AFTERHOURS: Using RTH LOD/HOD (no recalculation in post-market)")
        ...
        if recent_bar_data and len(recent_bar_data) > 0:
            ...
            bars_count = 0
            rth_only_data = {}
            for data in range(len(recent_bar_data)):
                bar_ts = recent_bar_data.get(data)
                bar_date_val = bar_ts.get('date') or bar_ts.get('dateTime')
                if bar_date_val is None:
                    continue
                if hasattr(bar_date_val, 'time'):
                    bar_time = bar_date_val.time()
                else:
                    try:
                        bar_time = datetime.datetime.strptime(str(bar_date_val)[:19], '%Y-%m-%d %H:%M:%S').time()
                    except Exception:
                        continue
                if rth_start <= bar_time < rth_end:
                    bar_low = float(bar_ts.get('low', 0))
                    bar_high = float(bar_ts.get('high', 0))
                    if bar_low < low_value:
                        low_value = bar_low
                    if bar_high > high_value:
                        high_value = bar_high
                    rth_only_data[bars_count] = {
                        'low': bar_low, 'high': bar_high,
                        'open': float(bar_ts.get('open', 0)), 'close': float(bar_ts.get('close', 0)),
                        'date': bar_date_val
                    }
                    bars_count += 1
            if low_value != float('inf') and high_value != float('-inf'):
                lod, hod = low_value, high_value
                if lod > hod:
                    lod, hod = hod, lod
                logging.info(f"RTH HOD/LOD (post-market): LOD={lod}, HOD={hod} (from {bars_count} RTH bars)")
                return lod, hod, rth_only_data
        ...
        if raw_hist_data and len(raw_hist_data) > 0:
            ...
            for i, bar in enumerate(raw_hist_data):
                ...
                if bar_date == today and rth_start <= bar_time < rth_end:
                    ...
                    if bar_high > high_value:
                        high_value = bar_high
                    recent_bar_data[bars_count] = {'low': bar_low, 'high': bar_high, 'open': float(bar.open), 'close': float(bar.close), 'date': bar.date}
                    bars_count += 1
            if low_value != float('inf') and high_value != float('-inf'):
                lod, hod = low_value, high_value
                if lod > hod:
                    lod, hod = hod, lod
                logging.info(f"RTH HOD/LOD (post-market fallback): LOD={lod}, HOD={hod} (from {bars_count} RTH bars)")
                return lod, hod, recent_bar_data
        logging.warning("AFTERHOURS: No RTH data available for LOD/HOD")
        return None, None, {}
...
async def SendTrade(connection, symbol,timeFrame,profit,stopLoss,risk,tif,barType,buySellType,atrPercentage,quantity,pullBackNo,slValue , breakEven , outsideRth,entry_points, option_enabled=False, option_contract="", option_expire="", option_entry_order_type="Market", option_sl_order_type="Market", option_tp_order_type="Market", option_risk_amount=""):
    try:
        ...
        logging.debug("SendTrade: Checking routing - barType='%s', PBe1='%s'", barType, Config.entryTradeType[Config.PBe1_INDEX] if len(Config.entryTradeType) > Config.PBe1_INDEX else "N/A")
        ...
        if outsideRth:
            if session in ('PREMARKET', 'AFTERHOURS'):
                ...
                allowed_types = (Config.entryTradeType[Config.CONDITIONAL_ORDER_INDEX],  # Conditional Order
        ...
        if barType == Config.entryTradeType[Config.FB_INDEX]:  # FB
            ...
            logging.debug(
                "Routing to pull_back_PBe1 for barType='%s' (PBe1='%s', PBe1 (3)='%s')",
                barType,
                Config.entryTradeType[Config.PBe1_INDEX],
                Config.entryTradeType[Config.PBe1_3_INDEX],
            )
            # PBe1 (3) uses the same logic as PBe1, but requires 3 consecutive bars
            require_3_consecutive_bars = barType == Config.entryTradeType[Config.PBe1_3_INDEX]
            await pull_back_PBe1(
                connection,
                symbol,
                timeFrame,
                profit,
                ...
                outsideRth,
                entry_points,
                require_3_consecutive_bars=require_3_consecutive_bars,
            )
        elif barType in (
            Config.entryTradeType[Config.PBe2_INDEX],
            Config.entryTradeType[Config.PBe2_3_INDEX],
        ):  # PBe2 / PBe2 (3)
            logging.debug(
                "Routing to pull_back_PBe2 for barType='%s' (PBe2='%s', PBe2 (3)='%s')",
                barType,
                Config.entryTradeType[Config.PBe2_INDEX],
                Config.entryTradeType[Config.PBe2_3_INDEX],
            )
            # PBe2 (3) uses the same logic as PBe2, but requires 3 consecutive bars
            require_3_consecutive_bars = barType == Config.entryTradeType[Config.PBe2_3_INDEX]
            await pull_back_PBe2(
                connection,
                symbol,
                timeFrame,
                profit,
...
def _get_pbe1_lod_hod(connection, contract, timeFrame, tradeType):
    """
    Get Low of Day (LOD) and High of Day (HOD) for the current trading session.
    LOD = minimum price in the session; HOD = maximum price in the session.
    
    - Premarket: uses premarket bars only (04:00 to 09:30).
    - RTH: uses RTH bars from 09:30 to current time (session so far).
    - Post-market (after hours): does not recalculate; uses RTH LOD/HOD only
      (bars from 09:30 to 16:00).
    ...
    try:
        ...
        # Post-market: use RTH LOD/HOD only (no recalculation with after-hours data)
        if session == 'AFTERHOURS':
            logging.info("PBe1 AFTERHOURS: Using RTH LOD/HOD (no recalculation in post-market)")
            chart_Time = datetime.datetime.strptime(str(today) + " " + Config.tradingTime, "%Y-%m-%d %H:%M:%S")
            recent_bar_data = connection.getHistoricalChartDataForEntry(contract, timeFrame, chart_Time)
            if recent_bar_data and len(recent_bar_data) > 0:
                low_value = float('inf')
                ...
                if low_value != float('inf') and high_value != float('-inf'):
                    lod, hod = low_value, high_value
                    if lod > hod:
                        logging.error(f"PBe1 RTH HOD/LOD ERROR: LOD ({lod}) > HOD ({hod})! Swapping values.")
                        lod, hod = hod, lod
                    logging.info(f"PBe1 RTH HOD/LOD (post-market): LOD={lod}, HOD={hod} (from {bars_count} RTH bars)")
                    return lod, hod
            raw_hist_data = connection.getChartData(contract, timeFrame, None)
            if raw_hist_data and len(raw_hist_data) > 0:
                low_value = float('inf')
                ...
                if low_value != float('inf') and high_value != float('-inf'):
                    ...
                    if lod > hod:
                        lod, hod = hod, lod
                    logging.info(f"PBe1 RTH HOD/LOD (post-market fallback): LOD={lod}, HOD={hod} (from {bars_count} RTH bars)")
                    return lod, hod
            logging.warning("PBe1 AFTERHOURS: No RTH data available for LOD/HOD")
            return 0, 0

        # Premarket: use premarket bars only (04:00 to 09:30)
        if session == 'PREMARKET':
            logging.info("PBe1 PREMARKET: Using premarket data for HOD/LOD calculation")
        raw_hist_data = connection.getChartData(contract, timeFrame, None)
        if raw_hist_data and len(raw_hist_data) > 0 and session == 'PREMARKET':
            low_value = float('inf')
            high_value = float('-inf')
            bars_count = 0
            for bar in raw_hist_data:
            ...
            if low_value != float('inf') and high_value != float('-inf'):
                ...
                hod = high_value
                if lod > hod:
                    logging.error(f"PBe1 PREMARKET HOD/LOD ERROR: LOD ({lod}) > HOD ({hod})! Swapping values.")
                    lod, hod = hod, lod
                logging.info(f"PBe1 PREMARKET HOD/LOD: LOD={lod}, HOD={hod} (from {bars_count} premarket bars)")
                return lod, hod
            logging.warning("PBe1 PREMARKET: No premarket bars found for today")

        # RTH: use RTH data (09:30 to current time)
        ...
        if raw_hist_data and len(raw_hist_data) > 0:
            ...
            if low_value != float('inf') and high_value != float('-inf'):
                ...
                hod = high_value
                if lod > hod:
                    logging.error(f"PBe1 RTH HOD/LOD (fallback) ERROR: LOD ({lod}) > HOD ({hod})! Swapping values.")
                    lod, hod = hod, lod
...
def _calculate_manual_stop_loss(connection, contract, entry_price, stopLoss, buySellType, timeFrame, slValue):
    ...
    if stopLoss == Config.stopLoss[0]:  # EntryBar
    ...
    elif stopLoss == Config.stopLoss[3]:  # HOD
        ...
        stop_size = abs(entry_price - stop_loss_price)
        if stop_size < 0.01:
            stop_size = max(0.01, (bar_high - bar_low) + Config.add002)
            logging.warning(f"Manual HOD: stop_size was 0 or very small (entry=stop); using min stop_size={stop_size}")
        logging.info(f"Manual HOD: entry={entry_price}, stop={stop_loss_price}, stop_size={stop_size}")
    
    elif stopLoss == Config.stopLoss[4]:  # LOD
        ...
        if stop_size < 0.01:
            stop_size = max(0.01, (bar_high - bar_low) + Config.add002)
            logging.warning(f"Manual LOD: stop_size was 0 or very small (entry=stop); using min stop_size={stop_size}")
...
def sendEntryTrade(connection,ibcontract,tradeType,quantity,histData,lastPrice, symbol,timeFrame,profit,stopLoss,risk,tif,barType,userBuySell,userAtr,slValue=0,breakEven=False ,outsideRth=False,entry_points='0'):
    try:
        logging.info(f"sendEntryTrade CALLED: barType='{barType}', Config.entryTradeType[Config.PBe1_INDEX]='{Config.entryTradeType[Config.PBe1_INDEX] if len(Config.entryTradeType) > 6 else 'N/A'}', Config.entryTradeType[Config.PBe2_INDEX]='{Config.entryTradeType[Config.PBe2_INDEX] if len(Config.entryTradeType) > 7 else 'N/A'}'")
        ...
        if barType == Config.entryTradeType[Config.FB_INDEX]:  # FB
            ...
            # For stop orders, add a safety buffer to TP to account for potential slippage
            # Stop orders can fill significantly above/below the trigger price
            # Calculate stop_size to determine appropriate buffer (use ATR if available, otherwise bar-based)
            try:
                # Use ATR stop_size if stopLoss is ATR-based, otherwise use bar-based
                if stopLoss in Config.atrStopLossMap:
                    atr_offset = _get_atr_stop_offset(connection, ibcontract, stopLoss)
                ...
                # Add a slippage buffer: 0.5 * stop_size to account for typical stop order slippage
                slippage_buffer = round(stop_size * 0.5, Config.roundVal)
                if tradeType == 'BUY':
                    # For BUY: ensure TP is at least slippage_buffer above the trigger price
                    min_tp = round(lastPrice + slippage_buffer, Config.roundVal)
                    if tp < min_tp:
                        logging.warning(f"FB TP adjustment: Original TP={tp} is below minimum (trigger={lastPrice} + buffer={slippage_buffer} = {min_tp}). Adjusting TP to {min_tp} to account for stop order slippage.")
                        tp = min_tp
                else:  # SELL
                    # For SELL: ensure TP is at least slippage_buffer below the trigger price
                    max_tp = round(lastPrice - slippage_buffer, Config.roundVal)
                    if tp > max_tp:
                        logging.warning(f"FB TP adjustment: Original TP={tp} is above maximum (trigger={lastPrice} - buffer={slippage_buffer} = {max_tp}). Adjusting TP to {max_tp} to account for stop order slippage.")
                        tp = max_tp
            except Exception as e:
                logging.warning(f"Could not calculate slippage buffer for FB TP: {e}. Using original TP={tp}")
            ...
            entry_kwargs = dict(
                ...
                action=ord.action,
                transmit=False  # Entry order should NOT transmit - only the last order (SL) should transmit
            )
            ...
            # Ensure TP order also has transmit=False (should already be set by bracketOrder, but verify)
            br_order[1].transmit = False
            # Ensure SL order has transmit=True (last order in bracket transmits the entire group)
            ...
            # Store TP/SL prices and order type info in orderStatusData for bracket order regeneration if Error 103 occurs
            entry_order_id = int(entry_res.order.orderId)
            if entry_order_id in Config.orderStatusData:
                Config.orderStatusData[entry_order_id]['tp_price'] = tp
                Config.orderStatusData[entry_order_id]['stop_loss_price'] = sl
                Config.orderStatusData[entry_order_id]['stop_order_type'] = stop_order_type  # Store 'STP' or 'STP LMT'
                if stop_order_type == 'STP LMT' and hasattr(stop_order, 'lmtPrice'):
                    Config.orderStatusData[entry_order_id]['stop_limit_price'] = stop_order.lmtPrice
                logging.info(f"FB: Stored TP/SL prices and order type in orderStatusData: tp={tp}, sl={sl}, stop_order_type={stop_order_type} for orderId={entry_order_id}")
            
            tp_res = connection.placeTrade(contract=ibcontract, order=br_order[1], outsideRth=outsideRth)
            logging.info(f"response of  bracket order second %s   contract %s", tp_res, ibcontract)
            StatusUpdate(tp_res, 'TakeProfit', ibcontract, 'LMT', tradeType, quantity, histData, lastPrice, symbol,
                         timeFrame, profit, stopLoss, risk, '', tif, barType, userBuySell, userAtr, slValue,
                         breakEven, outsideRth)
            sl_res = connection.placeTrade(contract=ibcontract, order=br_order[2], outsideRth=outsideRth)
            logging.info(f"response of  bracket order third %s   contract %s", sl_res, ibcontract)
            StatusUpdate(sl_res, 'StopLoss', ibcontract, stop_order_type, tradeType, quantity, histData, lastPrice, symbol,
            ...
            # Store TP/SL order IDs in orderStatusData for potential future updates
            if entry_order_id in Config.orderStatusData:
                if tp_res and tp_res.order:
                    Config.orderStatusData[entry_order_id]['tp_order_id'] = int(tp_res.order.orderId)
                if sl_res and sl_res.order:
                    Config.orderStatusData[entry_order_id]['sl_order_id'] = int(sl_res.order.orderId)
                logging.info(f"FB: Stored TP/SL order IDs in orderStatusData: tp_order_id={Config.orderStatusData[entry_order_id].get('tp_order_id')}, sl_order_id={Config.orderStatusData[entry_order_id].get('sl_order_id')}")

        elif barType == Config.entryTradeType[1] or barType == Config.entryTradeType[Config.CONDITIONAL_ORDER_INDEX]:
        ...
        elif barType == Config.entryTradeType[Config.PBe2_INDEX]:  # PBe2 - same logic as PBe1
            logging.info(f"sendEntryTrade: PBe2 block reached! barType='{barType}', Config.entryTradeType[Config.PBe2_INDEX]='{Config.entryTradeType[Config.PBe2_INDEX]}'")
            ...
            if is_extended:
                ...
                if stopLoss == Config.stopLoss[1]:  # Custom
                    ...
                    # For HOD/LOD: Limit = entry_stop_price ± multiplier*stop_size (same as Custom, consistent with user request)
                    if tradeType == 'BUY':
                        limit_price = round(entry_stop_price + entry_limit_offset, Config.roundVal)
                    else:  # SELL
                        limit_price = round(entry_stop_price - entry_limit_offset, Config.roundVal)
                    logging.info(f"PBe2 Extended hours HOD/LOD {tradeType}: Stop={entry_stop_price}, Limit={limit_price} (entry_stop ± 0.5×stop_size={entry_limit_offset}), LOD={lod}, HOD={hod}")
        ...
        elif barType == Config.entryTradeType[Config.PBe1_INDEX]:  # PBe1 - similar to RBB+LOD/HOD
            logging.info(f"sendEntryTrade: PBe1 block reached! barType='{barType}', Config.entryTradeType[Config.PBe1_INDEX]='{Config.entryTradeType[Config.PBe1_INDEX]}'")
...
def get_tp_for_selling(connection, timeFrame, contract, profit_type, filled_price, histData, stopLoss=None):
    # Take-profit levels for long positions (sell orders) use the configured stop size.
    # Note: filled_price parameter is actually the entry stop order price (stop entry price/trigger price), not the actual fill price
    stop_size = None
    
    # For ATR stop loss, use ATR stop_size instead of bar-based stop_size
    if stopLoss and stopLoss in Config.atrStopLossMap:
        atr_offset = _get_atr_stop_offset(connection, contract, stopLoss)
        if atr_offset is not None and atr_offset > 0:
            stop_size = atr_offset
            logging.info(f"get_tp_for_selling: Using ATR stop_size={stop_size} for TP calculation (stopLoss={stopLoss})")
        else:
            logging.warning(f"get_tp_for_selling: ATR unavailable for {stopLoss}, falling back to bar-based stop_size")
            try:
                stop_size, _, _ = _calculate_stop_limit_offsets(histData)
            except Exception:
                high = float(histData['high'])
        ...
        # For non-ATR stop loss, use bar-based stop_size
        try:
            stop_size, _, _ = _calculate_stop_limit_offsets(histData)
        except Exception:
            high = float(histData['high'])
    ...
    if multiplier is not None:
        # Take Profit = stop entry price + multiplier * stop size (for BUY, TP is above entry)
        # stop entry price = entry stop order trigger price (auxPrice), not the actual fill price
    ...
    logging.info(
        f"tp calculation for selling (BUY trade) %s , price %s histData %s stop_entry_price %s profit_type %s stop_size %s multiplier %s (formula: stop_entry_price + multiplier * stop_size)",
        contract, price, histData, filled_price, profit_type, stop_size, multiplier,
    )
    return price


def get_tp_for_buying(connection, timeFrame, contract, profit_type, filled_price, histData, stopLoss=None):
    ...
    if stopLoss and stopLoss in Config.atrStopLossMap:
        ...
        if atr_offset is not None and atr_offset > 0:
            stop_size = atr_offset
            logging.info(f"get_tp_for_buying: Using ATR stop_size={stop_size} for TP calculation (stopLoss={stopLoss})")
        else:
            logging.warning(f"get_tp_for_buying: ATR unavailable for {stopLoss}, falling back to bar-based stop_size")
            try:
                stop_size, _, _ = _calculate_stop_limit_offsets(histData)
            except Exception:
                high = float(histData['high'])
    ...
    multiplier = multiplier_map.get(profit_type)
    if multiplier is not None:
        # Take Profit = stop entry price - multiplier * stop size (for SELL, TP is below entry)
        # stop entry price = entry stop order trigger price (auxPrice), not the actual fill price
        price = float(filled_price) - (multiplier * stop_size)
    else:
    ...
    price = round(price, Config.roundVal)
    logging.info(
        f"tp calculation for buying (SELL trade) %s , price %s histData %s stop_entry_price %s profit_type %s stop_size %s multiplier %s (formula: stop_entry_price - multiplier * stop_size)",
        contract, price, histData, filled_price, profit_type, stop_size, multiplier,
    )
    return price
...
def get_sl_for_selling(connection,stoploss_type,filled_price, histData,slValue ,contract,timeframe,chartTime):
    ...
    # Note: filled_price parameter is actually the entry stop order price (stop entry price/trigger price), not the actual fill price
    # For BUY: Stop Loss = stop entry price - stop size
    if stoploss_type in Config.atrStopLossMap:
        ...
        if atr_offset is not None:
            # Stop Loss = stop entry price - stop size (for BUY, SL is below entry)
            # stop entry price = entry stop order trigger price (auxPrice), not the actual fill price
            stpPrice = float(filled_price) - atr_offset
            logging.info("ATR stop loss (long) %s offset %s stop_entry_price %s stop %s (formula: stop_entry_price - stop_size)",
...
def get_sl_for_buying(connection,stoploss_type,filled_price, histData,slValue ,contract,timeframe,chartTime):
    # we are sending buy sl order (for SELL entry)
    # Note: filled_price parameter is actually the entry stop order price (stop entry price/trigger price), not the actual fill price
    # For SELL: Stop Loss = stop entry price + stop size
    if stoploss_type in Config.atrStopLossMap:
        atr_offset = _get_atr_stop_offset(connection, contract, stoploss_type)
        if atr_offset is not None:
            # Stop Loss = stop entry price + stop size (for SELL, SL is above entry)
            # stop entry price = entry stop order trigger price (auxPrice), not the actual fill price
            stpPrice = float(filled_price) + atr_offset
            logging.info("ATR stop loss (short) %s offset %s stop_entry_price %s stop %s (formula: stop_entry_price + stop_size)",
                         stoploss_type, atr_offset, filled_price, stpPrice)
            stpPrice = round(stpPrice, Config.roundVal)
            return stpPrice
        else:
...
def TpSlForFB(connection,ibcontract,tradeType,quantity,histData,lastPrice, symbol,timeFrame,profit,stopLoss,risk,tif,barType,userBuySell,userAtr,slValue=0,breakEven=False ,outsideRth=False):
    # lastPrice is the entry stop order price (stop entry price/trigger price), used for TP/SL calculations
    # For BUY: TP = stop_entry_price + multiplier * stop_size, SL = stop_entry_price - stop_size
    # For SELL: TP = stop_entry_price - multiplier * stop_size, SL = stop_entry_price + stop_size
    try:
        tp,sl=0,0
        if tradeType == 'BUY':
...
async def sendTpSlBuy(connection, entryData):
    try:
        ...
        while True:
            ...
            if entryData.get('barType') == Config.entryTradeType[Config.PBe1_INDEX] or entryData.get('barType') == Config.entryTradeType[Config.PBe2_INDEX]:  # PBe1 or PBe2
            ...
            if (entryData['barType'] == Config.entryTradeType[Config.CONDITIONAL_ORDER_INDEX]) or (entryData['barType'] == Config.entryTradeType[Config.FB_INDEX]) or (entryData['barType'] == Config.entryTradeType[Config.RB_INDEX]) or (entryData['barType'] == Config.entryTradeType[Config.RBB_INDEX]) or (entryData['barType'] == Config.entryTradeType[Config.PBe1_INDEX]) or (entryData['barType'] == Config.entryTradeType[Config.PBe2_INDEX]) or (entryData['barType'] == Config.entryTradeType[Config.LB2_INDEX]) or (entryData['barType'] == Config.entryTradeType[Config.LB3_INDEX]):
                ...
                # PBe1/PBe2 (including PBe1 (3)/PBe2 (3)): Always uses HOD/LOD for stop loss (similar to RBB+HOD/LOD)
                stop_loss_type = entryData.get('stopLoss')
                # Treat PBe1 (3) the same as PBe1, and PBe2 (3) the same as PBe2
                is_pbe1 = entryData['barType'] in (
                    Config.entryTradeType[Config.PBe1_INDEX],
                    Config.entryTradeType[Config.PBe1_3_INDEX],
                )
                is_pbe2 = entryData['barType'] in (
                    Config.entryTradeType[Config.PBe2_INDEX],
                    Config.entryTradeType[Config.PBe2_3_INDEX],
                )
                ...
                if is_pbe1_or_pbe2:  # PBe1 or PBe2 - always uses HOD/LOD for stop loss (similar to RBB+HOD/LOD)
                ...
                # PBe2: Already handled above in is_pbe1_or_pbe2 block (same TP logic as PBe1) - no separate block needed
                # to avoid duplicate TP send
                if not is_pbe2:
                ...
                if entryData['barType'] in Config.manualOrderTypes:
                    ...
                    # Skip further processing for custom stop loss - price is already set
                    elif entryData['barType'] == 'Custom' and stop_loss_type in Config.atrStopLossMap:  # Custom entry type with ATR stop loss
                        # For Custom entry type with ATR stop loss: use stored stop_size or recalculate ATR
                        order_id = entryData.get('orderId')
                        order_data = Config.orderStatusData.get(order_id) if order_id else None
                        stored_stop_size = order_data.get('stopSize') if order_data else None
                        ...
                        if stored_stop_size is not None and stored_stop_size > 0:
                            ...
                            # Recalculate ATR stop_size if not stored
                            atr_offset = _get_atr_stop_offset(connection, entryData['contract'], stop_loss_type)
                            if atr_offset is not None and atr_offset > 0:
                                stop_size = atr_offset
                            else:
                        ...
                        if stop_size is not None and stop_size > 0:
                            ...
                            price = float(entry_stop_price) - (multiplier * stop_size)
                            price = round(price, Config.roundVal)
                            logging.info(f"Manual Order Custom+ATR TP (SHORT): entry={entry_stop_price}, stop_size={stop_size} (ATR, {'stored' if stored_stop_size else 'recalculated'}), stopLoss={stop_loss_type}, multiplier={multiplier}, tp={price}")
                        else:
            ...
            if price == 0 or price is None:
                logging.warning("Manual Order: TP price is 0 or None, checking fallback options. barType=%s, stopLoss=%s", 
                ...
                if entryData.get('barType') in Config.manualOrderTypes and entryData.get('stopLoss') == Config.stopLoss[1]:
                    ...
                    if custom_stop > 0 and entry_stop_price > 0:
                        ...
                        price = None
                # Try to calculate fallback TP for Custom barType + ATR stop loss
                elif entryData.get('barType') == 'Custom' and entryData.get('stopLoss') in Config.atrStopLossMap:
                    # Try to get stored stop_size first
                    order_id = entryData.get('orderId')
                    order_data = Config.orderStatusData.get(order_id) if order_id else None
                    stored_stop_size = order_data.get('stopSize') if order_data else None
                    ...
                    if stored_stop_size is not None and stored_stop_size > 0:
                        ...
                        # Recalculate ATR stop_size
                        atr_offset = _get_atr_stop_offset(connection, entryData['contract'], entryData.get('stopLoss'))
                        if atr_offset is not None and atr_offset > 0:
                            stop_size = atr_offset
                        else:
                            stop_size = None
                    
                    if stop_size is not None and stop_size > 0 and entry_stop_price > 0:
                        multiplier_map = {
                            Config.takeProfit[0]: 1,    # 1:1
                            Config.takeProfit[1]: 1.5,  # 1.5:1
                            Config.takeProfit[2]: 2,    # 2:1
                        ...
                        price = float(entry_stop_price) - (multiplier * stop_size)
                        price = round(price, Config.roundVal)
                        source = 'stored' if stored_stop_size else 'recalculated'
                        logging.warning("Manual Order Custom+ATR: Recalculated TP using fallback: entry=%s, stop_size=%s (ATR, %s), multiplier=%s, price=%s", 
                                      entry_stop_price, stop_size, source, multiplier, price)
                    else:
                        logging.error("Manual Order Custom+ATR: Cannot calculate fallback TP - stop_size=%s, entry_stop_price=%s", 
                                    stop_size, entry_stop_price)
                        price = None
                else:
                    logging.error("Manual Order: Cannot calculate TP, skipping TP order. barType=%s, stopLoss=%s", 
                                entryData.get('barType'), entryData.get('stopLoss'))
                    price = None
                # Fallback TP for Limit-style entries (Limit Order / ASK + 1/2 / BID - 1/2) with ATR stop loss
                # This covers cases where the earlier limit-style ATR TP branch did not run (e.g. unexpected control flow)
                if (
                    (price == 0 or price is None)
                    and _is_limit_style_entry(entryData.get('barType'))
                    and entryData.get('stopLoss') in Config.atrStopLossMap
                ):
                    atr_offset = _get_atr_stop_offset(connection, entryData['contract'], entryData.get('stopLoss'))
                    if atr_offset is not None and atr_offset > 0 and entry_stop_price > 0:
                        stop_size = atr_offset
                        multiplier_map = {
                            Config.takeProfit[0]: 1,    # 1:1
                            Config.takeProfit[1]: 1.5,  # 1.5:1
                        ...
                        price = round(price, Config.roundVal)
                        logging.warning(
                            "Manual Order Limit-style+ATR: Recalculated TP using fallback: entry=%s, stop_size=%s (ATR), multiplier=%s, tp=%s, barType=%s",
                            entry_stop_price, stop_size, multiplier, price, entryData.get('barType')
                        )
                    else:
                        logging.error(
                            "Manual Order Limit-style+ATR: Cannot calculate fallback TP - atr_offset=%s, entry_stop_price=%s, barType=%s",
                            atr_offset, entry_stop_price, entryData.get('barType')
                        )
            
            # Skip TP send for PBe1/PBe2 - they already sent TP above
            is_pbe1 = (entryData['barType'] == Config.entryTradeType[Config.PBe1_INDEX])  # PBe1
            is_pbe2 = (entryData['barType'] == Config.entryTradeType[Config.PBe2_INDEX])  # PBe2
            ...
            if (entryData['barType'] == Config.entryTradeType[Config.PBe1_INDEX] or entryData['barType'] == Config.entryTradeType[Config.PBe2_INDEX]):  # PBe1 or PBe2
                bar_type_name = "PBe1" if entryData['barType'] == Config.entryTradeType[Config.PBe1_INDEX] else "PBe2"
            ...
            # Note: RBB (Config.RBB_INDEX) uses different logic - it updates stop price continuously via rbb_loop_run, but still needs initial LOD/HOD calculation
                ...
                if stop_loss_type == Config.stopLoss[0]:  # EntryBar
                    ...
                    if entryData.get('barType') == Config.entryTradeType[Config.RBB_INDEX]:  # RBB
                        ...
                        logging.info(f"RBB EntryBar stop loss (for SHORT): bar_high={bar_high}, stpPrice={stpPrice} (bar_high + 0.01), stop_size={stop_size}, barType={entryData.get('barType')}")
                    else:
...
def sendStopLoss(connection, entryData,price,action):
    try:
        ...
        order_type = "STP"
        # Used for extended-hours STP LMT stop-loss legs.
        # Must be initialized to avoid UnboundLocalError in paths that compute stop_size but forget limit_price.
        limit_price = None
        # OCA group so TP and SL cancel each other; do NOT set parentId - entry is already filled,
        # so IB would reject with "Parent order is being cancelled." (Error 201)
        oca_id = entryData.get('orderId')
        order_kwargs = dict(
            orderType="STP",
            action=action,
            totalQuantity=entryData['totalQuantity'],
            tif=entryData['tif'],
            ocaGroup="tp" + str(oca_id),
            ocaType=1
        )
        ...
        if not is_extended and bar_type in [Config.entryTradeType[Config.PBe1_INDEX], Config.entryTradeType[Config.PBe2_INDEX]]:  # PBe1 or PBe2 in regular hours
            bar_type_name = "PBe1" if bar_type == Config.entryTradeType[Config.PBe1_INDEX] else "PBe2"
        ...
        if extended_hist_supported and is_extended:
            ...
            if stop_loss_type in Config.atrStopLossMap:
                ...
                if atr_offset is not None:
                    ...
                    limit_offset = EXTENDED_HOURS_SL_LIMIT_OFFSET_MULT * atr_offset  # premarket/postmarket: limit = stop ± 3×stop_size
                    logging.info(f"Extended hours ATR stop-limit: action={action}, ATR stop_size={stop_size}, protection_offset={protection_offset}, limit_offset={limit_offset} (3×stop_size)")
                else:
                ...
                if calculated_stop_size is not None and calculated_stop_size > 0:
                    ...
                    stop_size = calculated_stop_size
                    # For RB in extended hours: stop = entry ± stop_size, limit = stop ± 3×stop_size (premarket/postmarket)
                    protection_offset = stop_size * 1.0  # 1x for stop price (already calculated in sendTpSlBuy/Sell)
                    limit_offset = stop_size * EXTENDED_HOURS_SL_LIMIT_OFFSET_MULT  # 3x for limit price
                    logging.info(f"RB/LB/LB2/LB3 Extended hours stop-limit: stop_size={stop_size}, protection_offset={protection_offset} (for stop), limit_offset={limit_offset} (3×stop_size)")
                else:
            ...
            if is_manual_stop_order:
                ...
                if stop_loss_type == Config.stopLoss[1]:  # 'Custom'
                    ...
                    # Premarket/postmarket: limit = stop ± 3×stop_size (Long/SELL: stop-3x, Short/BUY: stop+3x)
                    limit_offset = round(manual_stop_size * EXTENDED_HOURS_SL_LIMIT_OFFSET_MULT, Config.roundVal)
                    if action.upper() == "SELL":
                        limit_price = round(stop_loss_price - limit_offset, Config.roundVal)
                        logging.info(f"Manual Stop Order Custom OTH: SELL stop loss (LONG): entry={filled_price}, stop={stop_loss_price}, limit={limit_price} (stop - 3×stop_size={limit_offset}), stop_size={manual_stop_size}")
                    else:
                        limit_price = round(stop_loss_price + limit_offset, Config.roundVal)
                        logging.info(f"Manual Stop Order Custom OTH: BUY stop loss (SHORT): entry={filled_price}, stop={stop_loss_price}, limit={limit_price} (stop + 3×stop_size={limit_offset}), stop_size={manual_stop_size}")
                    ...
                    if stop_loss_type == Config.stopLoss[3] or stop_loss_type == Config.stopLoss[4]:  # HOD or LOD
                        ...
                        if action.upper() == "SELL":
                            limit_price = round(stop_loss_price - limit_offset, Config.roundVal)
                            logging.info(f"Custom entry OTH HOD/LOD: SELL stop loss (LONG): entry={filled_price}, stop={stop_loss_price} (LOD), limit={limit_price} (stop - 3×stop_size={limit_offset}), stop_size={manual_stop_size}")
                        else:
                            limit_price = round(stop_loss_price + limit_offset, Config.roundVal)
                            logging.info(f"Custom entry OTH HOD/LOD: BUY stop loss (SHORT): entry={filled_price}, stop={stop_loss_price} (HOD), limit={limit_price} (stop + 3×stop_size={limit_offset}), stop_size={manual_stop_size}")
                        ...
                        if action.upper() == "SELL":
                            limit_price = round(stop_loss_price - limit_offset, Config.roundVal)
                            logging.info(f"Custom entry OTH ATR: SELL stop loss (LONG): entry={filled_price}, stop={stop_loss_price}, limit={limit_price} (stop - 3×stop_size={limit_offset}), stop_size={manual_stop_size}")
                        else:
                            limit_price = round(stop_loss_price + limit_offset, Config.roundVal)
                            logging.info(f"Custom entry OTH ATR: BUY stop loss (SHORT): entry={filled_price}, stop={stop_loss_price}, limit={limit_price} (stop + 3×stop_size={limit_offset}), stop_size={manual_stop_size}")
                ...
                if bar_type in [Config.entryTradeType[Config.PBe1_INDEX], Config.entryTradeType[Config.PBe2_INDEX]]:  # PBe1 or PBe2
                    ...
                    if order_data and 'pbe1_lod' in order_data and 'pbe1_hod' in order_data:
                        ...
                        # For LONG position (SELL stop loss): use LOD
                        # For SHORT position (BUY stop loss): use HOD
                        if action.upper() == "SELL":  # LONG position: limit = stop - 3×stop_size
                            stop_loss_price = round(lod, Config.roundVal)
                            limit_offset = round(stored_stop_size * EXTENDED_HOURS_SL_LIMIT_OFFSET_MULT, Config.roundVal)
                            limit_price = round(stop_loss_price - limit_offset, Config.roundVal)
                            logging.info(f"{bar_type_name} stop loss (LONG) Extended hours: stop={stop_loss_price} (LOD), limit={limit_price} (stop - 3×stop_size={limit_offset}), entry={filled_price}, stop_size={stored_stop_size}")
                        else:  # BUY stop loss (SHORT position): limit = stop + 3×stop_size
                            stop_loss_price = round(hod, Config.roundVal)
                            limit_offset = round(stored_stop_size * EXTENDED_HOURS_SL_LIMIT_OFFSET_MULT, Config.roundVal)
                            limit_price = round(stop_loss_price + limit_offset, Config.roundVal)
                            logging.info(f"{bar_type_name} stop loss (SHORT) Extended hours: stop={stop_loss_price} (HOD), limit={limit_price} (stop + 3×stop_size={limit_offset}), entry={filled_price}, stop_size={stored_stop_size}")
                ...
                # Conditional Order, RB, RBB, LB, LB2, LB3: use existing logic
                    ...
                    if calculated_stop_size is not None:
                        ...
                        if lod_hod_stop_price is not None and lod_hod_stop_price > 0:
                            ...
                            stop_size = calculated_stop_size
                            # Premarket/postmarket: limit = stop ± 3×stop_size (Long/SELL: stop-3x, Short/BUY: stop+3x)
                            limit_offset = round(stop_size * EXTENDED_HOURS_SL_LIMIT_OFFSET_MULT, Config.roundVal)
                            stop_loss_price = round(price, Config.roundVal)
                            if action.upper() == "BUY":
                                limit_price = round(lod_hod_stop_price + limit_offset, Config.roundVal)
                                logging.info(f"RB/LB/LB2/LB3 HOD/LOD BUY stop loss (SHORT) Extended hours: stop={stop_loss_price}, limit={limit_price} (stop + 3×stop_size={limit_offset}), HOD={lod_hod_stop_price}, stop_size={stop_size}, entry={filled_price}")
                            else:
                                limit_price = round(lod_hod_stop_price - limit_offset, Config.roundVal)
                                logging.info(f"RB/LB/LB2/LB3 HOD/LOD SELL stop loss (LONG) Extended hours: stop={stop_loss_price}, limit={limit_price} (stop - 3×stop_size={limit_offset}), LOD={lod_hod_stop_price}, stop_size={stop_size}, entry={filled_price}")
                            ...
                            if stop_loss_type == Config.stopLoss[1]:  # 'Custom'
                                ...
                                if custom_stop == 0:
                                    ...
                                    if action.upper() == "BUY":
                                        ...
                                        limit_price = round(stop_loss_price + stop_size * EXTENDED_HOURS_SL_LIMIT_OFFSET_MULT, Config.roundVal)
                                    else:
                                        stop_loss_price = filled_price - stop_size
                                        limit_price = round(stop_loss_price - stop_size * EXTENDED_HOURS_SL_LIMIT_OFFSET_MULT, Config.roundVal)
                                    logging.warning(f"RB/LB/LB2/LB3/RBB Custom OTH: Custom stop loss value missing, using calculated stop_loss_price={stop_loss_price}, limit=stop ± 3×stop_size")
                                else:
                                    ...
                                    limit_offset = round(custom_stop_size * EXTENDED_HOURS_SL_LIMIT_OFFSET_MULT, Config.roundVal)
                                    if action.upper() == "BUY":
                                        limit_price = round(stop_loss_price + limit_offset, Config.roundVal)
                                        entry_price_for_log = entryData.get('lastPrice', filled_price)
                                        logging.info(f"RB/LB/LB2/LB3/RBB Custom OTH: BUY stop loss (SHORT): entry={entry_price_for_log}, stop={stop_loss_price}, limit={limit_price} (stop + 3×stop_size={limit_offset}), stop_size={custom_stop_size}")
                                    else:
                                        limit_price = round(stop_loss_price - limit_offset, Config.roundVal)
                                        entry_price_for_log = entryData.get('lastPrice', filled_price)
                                        logging.info(f"RB/LB/LB2/LB3/RBB Custom OTH: SELL stop loss (LONG): entry={entry_price_for_log}, stop={stop_loss_price}, limit={limit_price} (stop - 3×stop_size={limit_offset}), stop_size={custom_stop_size}")
                                ...
                                # Non-Custom: limit = stop ± 3×stop_size (stop is adjusted_price)
                                stop_size = calculated_stop_size if calculated_stop_size is not None else stop_size
                                limit_offset = round(stop_size * EXTENDED_HOURS_SL_LIMIT_OFFSET_MULT, Config.roundVal)
                                if action.upper() == "BUY":
                                    limit_price = round(adjusted_price + limit_offset, Config.roundVal)
                                    logging.info(f"RB/LB/LB2/LB3/RBB BUY stop loss (SHORT): stop={adjusted_price}, limit={limit_price} (stop + 3×stop_size={limit_offset}), entry={filled_price}")
                                else:
                                    limit_price = round(adjusted_price - limit_offset, Config.roundVal)
                                    logging.info(f"RB/LB/LB2/LB3/RBB SELL stop loss (LONG): stop={adjusted_price}, limit={limit_price} (stop - 3×stop_size={limit_offset}), entry={filled_price}")
                                order_kwargs['lmtPrice'] = round(limit_price, Config.roundVal)
                else:
                    ...
                    if bar_type == 'Limit Order':
                        ...
                        if stop_loss_type == Config.stopLoss[1]:  # 'Custom'
                            ...
                            if custom_stop == 0:
                                ...
                                if limit_order_stop_size is not None and limit_order_stop_size > 0:
                                    if action.upper() == "SELL":
                                        ...
                                        stop_loss_price = filled_price + limit_order_stop_size
                                    limit_offset = limit_order_stop_size * EXTENDED_HOURS_SL_LIMIT_OFFSET_MULT
                                    limit_price = round(stop_loss_price - limit_offset, Config.roundVal) if action.upper() == "SELL" else round(stop_loss_price + limit_offset, Config.roundVal)
                                    logging.warning(f"Limit Order Custom OTH: Custom stop loss value missing, using calculated stop_loss_price={stop_loss_price}, limit=stop ± 3×stop_size")
                                else:
                                    # Use bar-based calculation as fallback
                                    stop_loss_price = adjusted_price
                                    limit_offset = stop_size * EXTENDED_HOURS_SL_LIMIT_OFFSET_MULT
                                    limit_price = round(adjusted_price - limit_offset, Config.roundVal) if action.upper() == "SELL" else round(adjusted_price + limit_offset, Config.roundVal)
                            else:
                                # Stop loss price is the custom value directly
                                stop_loss_price = round(custom_stop, Config.roundVal)
                                ...
                                if action.upper() == "SELL":
                                    limit_price = round(stop_loss_price - limit_offset, Config.roundVal)
                                    logging.info(f"Limit Order Custom OTH: SELL stop loss (LONG): entry={filled_price}, stop={stop_loss_price}, limit={limit_price} (stop - 3×stop_size={limit_offset}), stop_size={manual_stop_size}")
                                else:
                                    limit_price = round(stop_loss_price + limit_offset, Config.roundVal)
                                    logging.info(f"Limit Order Custom OTH: BUY stop loss (SHORT): entry={filled_price}, stop={stop_loss_price}, limit={limit_price} (stop + 3×stop_size={limit_offset}), stop_size={manual_stop_size}")
                            ...
                            if stop_loss_type == Config.stopLoss[3] or stop_loss_type == Config.stopLoss[4]:  # HOD or LOD
                                ...
                                if action.upper() == "SELL":
                                    limit_price = round(stop_loss_price - limit_offset, Config.roundVal)
                                    logging.info(f"Limit Order HOD/LOD SELL stop loss (LONG) Extended hours: entry={filled_price}, stop={stop_loss_price} (LOD), limit={limit_price} (stop - 3×stop_size={limit_offset}), stop_size={manual_stop_size}")
                                else:
                                    limit_price = round(stop_loss_price + limit_offset, Config.roundVal)
                                    logging.info(f"Limit Order HOD/LOD BUY stop loss (SHORT) Extended hours: entry={filled_price}, stop={stop_loss_price} (HOD), limit={limit_price} (stop + 3×stop_size={limit_offset}), stop_size={manual_stop_size}")
                                ...
                                if limit_order_stop_size is not None and limit_order_stop_size > 0:
                                    ...
                                    if action.upper() == "BUY":
                                        limit_price = round(adjusted_price + limit_offset, Config.roundVal)
                                        logging.info(f"Limit Order BUY stop loss (SHORT): stop={adjusted_price}, limit={limit_price} (stop + 3×stop_size={limit_offset}), entry={filled_price}")
                                    else:
                                        limit_price = round(adjusted_price - limit_offset, Config.roundVal)
                                        logging.info(f"Limit Order SELL stop loss (LONG): stop={adjusted_price}, limit={limit_price} (stop - 3×stop_size={limit_offset}), entry={filled_price}")
                                else:
                                    # Limit Order without stored stop_size: fall through to original logic below
                                    pass
                    else:
                        # Other trade types: limit = stop ± 3×stop_size (premarket/postmarket)
                        limit_offset = round(stop_size * EXTENDED_HOURS_SL_LIMIT_OFFSET_MULT, Config.roundVal)
                        if action.upper() == "BUY":
                            limit_price = round(adjusted_price - limit_offset, Config.roundVal)
                            logging.info(f"BUY stop loss (SHORT position): stop={adjusted_price}, limit={limit_price} (stop - 3×stop_size={limit_offset})")
                        else:
                            limit_price = round(adjusted_price + limit_offset, Config.roundVal)
                            logging.info(f"SELL stop loss (LONG position): stop={adjusted_price}, limit={limit_price} (stop + 3×stop_size={limit_offset})")

            # For extended hours, use STP LMT (Stop-Limit Order) for stop loss
            # For RTH, use simple STP order (no limit price)
            if is_extended:
                # Ensure limit_price is computed for extended hours. Some branches above may not set it.
                if limit_price is None:
                    if 'stop_size' not in locals() or stop_size is None or stop_size <= 0:
                        stop_size = float(adjusted_price) * 0.01  # last-resort fallback
                    limit_offset = round(float(stop_size) * EXTENDED_HOURS_SL_LIMIT_OFFSET_MULT, Config.roundVal)
                    if action.upper() == "SELL":
                        # LONG position stop-loss: limit below stop
                        limit_price = round(float(adjusted_price) - limit_offset, Config.roundVal)
                    else:
                        # SHORT position stop-loss: limit above stop
                        limit_price = round(float(adjusted_price) + limit_offset, Config.roundVal)
                    logging.warning(
                        "sendStopLoss: Computed missing limit_price for extended hours: action=%s stop=%s limit=%s (stop ± 3×stop_size=%s) barType=%s",
                        action, adjusted_price, limit_price, limit_offset, entryData.get('barType', '')
                    )
                order_type = "STP LMT"
                order_kwargs['orderType'] = "STP LMT"
                order_kwargs['lmtPrice'] = round(limit_price, Config.roundVal)
            ...
            order_kwargs['lmtPrice'] = round(limit_price, Config.roundVal)
            logging.info(
                "Extended hours fallback stop-limit: action=%s stop=%s limit=%s (stop ± 3×stop_size=%s) session=%s",
                action, adjusted_price, order_kwargs['lmtPrice'], limit_offset, session
            )

        # Safety net: In extended hours we MUST place STP LMT.
        # Some paths above compute stop_size but forget to set lmtPrice/orderType, which causes missing SL or UnboundLocalError.
        if is_extended and 'lmtPrice' not in order_kwargs:
            if 'stop_size' not in locals() or stop_size is None or stop_size <= 0:
                # last-resort fallback: 1% of price
                stop_size = float(order_kwargs['auxPrice']) * 0.01
            limit_offset = round(float(stop_size) * EXTENDED_HOURS_SL_LIMIT_OFFSET_MULT, Config.roundVal)
            if action.upper() == "SELL":
                limit_price = round(float(order_kwargs['auxPrice']) - limit_offset, Config.roundVal)
            else:
                limit_price = round(float(order_kwargs['auxPrice']) + limit_offset, Config.roundVal)
            order_type = "STP LMT"
            order_kwargs['orderType'] = "STP LMT"
            order_kwargs['lmtPrice'] = limit_price
            logging.warning(
                "sendStopLoss: Extended-hours safety net applied: action=%s stop=%s limit=%s (stop ± 3×stop_size=%s) barType=%s",
                action, order_kwargs['auxPrice'], limit_price, limit_offset, entryData.get('barType', '')
            )
...
def sendTakeProfit(connection, entryData,price,action):
    try:
        ...
        if entryData['profit'] == Config.takeProfit[4] and not is_fixed_tp:
            ...
            logging.info(f"sendTakeProfit: NOT starting takeProfitThread - fixed TP (formula-based), barType={entryData.get('barType')}, stopLoss={entryData.get('stopLoss')}")
    except Exception as e:
    ...
    try:
        ...
        while True:
            ...
            if (entryData['barType'] == Config.entryTradeType[Config.CONDITIONAL_ORDER_INDEX]) or (entryData['barType'] == Config.entryTradeType[Config.FB_INDEX]) or (entryData['barType'] == Config.entryTradeType[Config.RB_INDEX]) or (entryData['barType'] == Config.entryTradeType[Config.RBB_INDEX]) or (entryData['barType'] == Config.entryTradeType[Config.PBe1_INDEX]) or (entryData['barType'] == Config.entryTradeType[Config.PBe2_INDEX]) or (entryData['barType'] == Config.entryTradeType[Config.LB2_INDEX]) or (entryData['barType'] == Config.entryTradeType[Config.LB3_INDEX]):
                ...
                stop_loss_type = entryData.get('stopLoss')
                is_pbe1_or_pbe2 = (entryData['barType'] == Config.entryTradeType[Config.PBe1_INDEX] or entryData['barType'] == Config.entryTradeType[Config.PBe2_INDEX])  # PBe1 or PBe2
                ...
                if not is_pbe1_or_pbe2:
                    ...
                    # Use entry_stop_price (entry trigger) for TP base: TP = entry + (multiplier × stop_size). Default 2:1 if profit key missing.
                    multiplier = multiplier_map.get(entryData['profit'], 2)
                    tp_base_price = entry_stop_price
                    price = float(tp_base_price) + (multiplier * stop_size)
                    is_extended, session = _is_extended_outside_rth(entryData.get('outsideRth', False))
                    logging.info(f"LB/RB/RBB/Conditional Order/LB2/LB3 TP (LONG): tp_base_price={tp_base_price} (entry_stop_price={entry_stop_price}, filled_price={filled_price}, is_extended={is_extended}), stop_size={stop_size}, multiplier={multiplier}, TP={price}")
            ...
            # For RB, RBB, LB, LB2, LB3, Conditional Order: calculate stop size in advance and use it for stop loss calculation
            # Note: RBB (Config.RBB_INDEX) uses different logic - it updates stop price continuously via rbb_loop_run, but still needs initial LOD/HOD calculation
            # PBe1/PBe2: Always uses HOD/LOD for stop loss (similar to RBB+HOD/LOD)
            # Conditional Order: Uses same logic as RB/RBB/LB for stop loss calculation
            ...
            if (entryData['barType'] == Config.entryTradeType[Config.PBe1_INDEX] or entryData['barType'] == Config.entryTradeType[Config.PBe2_INDEX]):  # PBe1 or PBe2
                ...
                # PBe1/PBe2: Ensure stop loss is on correct side of entry (LOD/HOD from premarket can be above entry for BUY or below for SELL)
                if entryData.get('barType') in (Config.entryTradeType[Config.PBe1_INDEX], Config.entryTradeType[Config.PBe2_INDEX]):
                    base_price = float(filled_price)
                    pbe_stop_size = entryData.get('calculated_stop_size') or 0
                    if entryData.get('action') == 'BUY' and pbe_stop_size > 0 and stpPrice > base_price:
                        stpPrice = round(base_price - pbe_stop_size, Config.roundVal)
                        logging.warning(f"PBe1/PBe2 Extended hours: LOD was above entry ({base_price}). Set stop loss below entry: stpPrice={stpPrice}")
                    elif entryData.get('action') == 'SELL' and pbe_stop_size > 0 and stpPrice < base_price:
                        stpPrice = round(base_price + pbe_stop_size, Config.roundVal)
                        logging.warning(f"PBe1/PBe2 Extended hours: HOD was below entry ({base_price}). Set stop loss above entry: stpPrice={stpPrice}")
                ...
                if stop_loss_type == Config.stopLoss[0]:  # EntryBar
                    ...
                    if stop_loss_type in Config.atrStopLossMap:
                        ...
                        if atr_offset is not None and atr_offset > 0:
                            stop_size = atr_offset
                            protection_offset = stop_size * 2.0
                            logging.info(f"RB/LB/LB2/LB3/Conditional Order ATR stop loss: stop_size={stop_size} (ATR offset), protection_offset={protection_offset}")
                        else:
                            # Fallback to bar range if ATR unavailable
                            try:
                                stop_size, _, protection_offset = _calculate_stop_limit_offsets(histData)
                                logging.warning(f"RB/LB/LB2/LB3/Conditional Order: ATR unavailable, using bar range stop_size={stop_size}")
                            except Exception:
                                stop_size = round((float(histData['high']) - float(histData['low']) + Config.add002), Config.roundVal)
                                protection_offset = stop_size * 2.0
                                logging.warning(f"RB/LB/LB2/LB3/Conditional Order: Using fallback stop_size={stop_size}")
                        ...
                        stpPrice = round(stpPrice, Config.roundVal)
                        entryData['calculated_stop_size'] = stop_size
                        logging.info(f"Conditional Order/RB/LB/LB2/LB3 ATR stop loss: base_price={base_price} (filled_price={filled_price}, entry_stop_price={entry_stop_price}), stop_size={stop_size}, stpPrice={stpPrice}, barType={entryData.get('barType')}, action={entryData.get('action')}, is_extended={is_extended}")
                    elif stop_loss_type == Config.stopLoss[1]:  # 'Custom'
                        # Use Custom stop loss value directly
                        custom_stop = _to_float(entryData.get('slValue', 0), 0)
                        if custom_stop == 0:
...
async def conditional_order(connection, symbol, timeFrame, profit, stopLoss, risk, tif, barType, buySellType,
    ...
    - Conditional Order 2: Both levels must be crossed in either order (cross-in-then-cross-out)
      - If price crosses above first price then crosses below second price → place Stop Order at stop_order_price
      - OR if price crosses below second price then crosses above first price → place Stop Order at stop_order_price
      - I.e. trigger only after both boundary crosses have occurred in sequence (either order).
    ...
    # For Conditional Order 2: track whether each boundary has been crossed (so we trigger only after both crossed in either order)
    co2_cond1_crossed = False
    co2_cond2_crossed = False
    ...
    while True:
        ...
        try:
            ...
            if ticker:
                try:
                    ...
                    if price is not None and price != 0 and not math.isnan(price):
                        ...
                        # Try last price
                        if hasattr(ticker, 'last') and ticker.last is not None and ticker.last != 0 and not math.isnan(ticker.last):
                            current_price = float(ticker.last)
                        # Try close price as fallback
                        elif hasattr(ticker, 'close') and ticker.close is not None and ticker.close != 0 and not math.isnan(ticker.close):
                            current_price = float(ticker.close)
                        else:
                            raise ValueError("Ticker price not available or is NaN")
                ...
                # Ticker not available, will use historical data
                current_price = None
            
            # Always fallback to historical data if ticker price is not available or is NaN
            if current_price is None or math.isnan(current_price):
                ...
                if histData and len(histData) > 0:
                    hist_price = float(histData[len(histData)-1].get('close', 0))
                    if hist_price and hist_price != 0 and not math.isnan(hist_price):
                        current_price = hist_price
                    else:
                        logging.info("Historical price data is invalid, retrying...")
                        await asyncio.sleep(1)
                        continue
                else:
            ...
            # Final check for NaN
            if current_price is None or math.isnan(current_price):
                logging.warning("Current price is NaN or None, retrying...")
                await asyncio.sleep(1)
                continue
        ...
        if selected_order == "1":
            ...
            # Conditional Order 2: Both levels must be crossed in either order.
            # Trigger when: (crossed above price1 then cross below price2) OR (crossed below price2 then cross above price1).
            cond1_met_now = False
            cond2_met_now = False
            
            # Check condition 1 this tick
            if condition1 == "Below":
                if current_price <= condition_price1:
                    co2_cond1_crossed = True
                    cond1_met_now = True
                    logging.info(f"CO2 condition 1 crossed: price {current_price} at/below condition_price1 {condition_price1}")
            elif condition1 == "Above":
                if current_price >= condition_price1:
                    co2_cond1_crossed = True
                    cond1_met_now = True
                    logging.info(f"CO2 condition 1 crossed: price {current_price} at/above condition_price1 {condition_price1}")
            
            # Check condition 2 this tick
            if condition2 == "Below":
                if current_price <= condition_price2:
                    co2_cond2_crossed = True
                    cond2_met_now = True
                    logging.info(f"CO2 condition 2 crossed: price {current_price} at/below condition_price2 {condition_price2}")
            elif condition2 == "Above":
                if current_price >= condition_price2:
                    co2_cond2_crossed = True
                    cond2_met_now = True
                    logging.info(f"CO2 condition 2 crossed: price {current_price} at/above condition_price2 {condition_price2}")
            
            # Trigger when we have crossed one boundary in the past and we are now crossing the other (either order)
            if (co2_cond1_crossed and cond2_met_now) or (co2_cond2_crossed and cond1_met_now):
                conditions_met = True
                logging.info(f"CO2 both levels crossed (either order): cond1_crossed={co2_cond1_crossed}, cond2_crossed={co2_cond2_crossed}, placing stop entry at {stop_order_price}")
        ...
        if conditions_met:
            ...
            # Get historical data FIRST (before stop loss calculation) to ensure EntryBar uses correct first bar
            # For EntryBar, we need the first bar of the current session, so use None to get most recent bar
            # For other stop loss types, we can also use None to get current bar data
            histData = None
            if stopLoss == Config.stopLoss[0]:  # EntryBar - need first bar of session
                # Try to get first bar from current day's data
                try:
                    # Get all bars from today to find the first one
                    recent_bars = connection.getHistoricalChartDataForEntry(contract, timeFrame, None)
                    if recent_bars and len(recent_bars) > 0:
                        # Get the first bar (oldest bar in the list)
                        first_bar = recent_bars[0]
                        histData = {
                            'high': float(first_bar.get('high', entry_price)),
                            'low': float(first_bar.get('low', entry_price)),
                            'close': float(first_bar.get('close', current_price)),
                            'open': float(first_bar.get('open', current_price)),
                            'dateTime': first_bar.get('dateTime', datetime.datetime.now())
                        }
                        logging.info("Conditional Order EntryBar: Found first bar from historical data: %s", histData)
                except Exception as e:
                    logging.warning("Failed to get first bar for EntryBar: %s", e)
            
            # If EntryBar didn't get data, or for other stop loss types, use standard method
            if not histData:
                histData = connection.getHistoricalChartData(contract, timeFrame, None)
            
            if not histData or len(histData) == 0:
                logging.warning("No historical data available for Conditional Order stop loss calculation, using fallback")
                histData = {
                    'high': entry_price,
                    'low': entry_price - 0.5 if buySellType == 'BUY' else entry_price + 0.5,
                    'close': current_price,
                    'open': current_price,
                    'dateTime': datetime.datetime.now()
                }
            ...
            if stopLoss == Config.stopLoss[1]:  # 'Custom'
                ...
                stop_loss_price = round(custom_stop, Config.roundVal)
                tp_base_price = entry_price  # Use entry_price for TP calculation
            elif stopLoss == Config.stopLoss[0]:  # EntryBar - use histData directly
                # EntryBar: stop = low (long) or high (short), stop_size = high-low+0.02
                bar_high = float(histData.get('high', entry_price))
                bar_low = float(histData.get('low', entry_price))
                if buySellType == 'BUY':
                    stop_loss_price = bar_low
                else:  # SELL
                    stop_loss_price = bar_high
                ...
                stop_loss_price = round(stop_loss_price, Config.roundVal)
                tp_base_price = entry_price
                logging.info("Conditional Order EntryBar: entry=%s, stop=%s, stop_size=%s (using first bar: high=%s, low=%s)", 
                           entry_price, stop_loss_price, stop_size, bar_high, bar_low)
            elif stopLoss in Config.atrStopLossMap:  # ATR-based stop loss
                # Use ATR offset for stop size
                atr_offset = _get_atr_stop_offset(connection, contract, stopLoss)
                if atr_offset is not None and atr_offset > 0:
                    stop_size = atr_offset
                    logging.info("Conditional Order ATR stop loss: entry=%s, ATR offset=%s, stop_size=%s", entry_price, atr_offset, stop_size)
                else:
                    # Fallback to bar range if ATR unavailable
                    bar_high = float(histData.get('high', entry_price))
                    bar_low = float(histData.get('low', entry_price))
                    stop_size = (bar_high - bar_low) + Config.add002
                    logging.warning("Conditional Order ATR: ATR unavailable, using bar range stop_size=%s", stop_size)
                
                stop_size = round(stop_size, Config.roundVal)
                # Calculate stop_loss_price from entry_price (not actual_entry_price) for ATR
                if buySellType == 'BUY':
                    stop_loss_price = entry_price - stop_size
                else:
                    stop_loss_price = entry_price + stop_size
                stop_loss_price = round(stop_loss_price, Config.roundVal)
                tp_base_price = entry_price
                logging.info("Conditional Order ATR: entry=%s, stop_size=%s, stop_loss_price=%s", entry_price, stop_size, stop_loss_price)
            else:
                # For other stop loss types (HOD, LOD, etc.), use _calculate_manual_stop_loss
                try:
                    raw_stop_loss_price, calculated_stop_size = _calculate_manual_stop_loss(
                        connection, contract, entry_price, stopLoss, buySellType, timeFrame, slValue
                    )
                ...
                # For non-Custom, non-EntryBar: SL = stop entry order ± stop_size (BUY: -, SELL: +)
                if buySellType == 'BUY':
                    stop_loss_price = entry_price - stop_size
                else:
                    stop_loss_price = entry_price + stop_size
            ...
            # Conditional Order RTH: TP = stop entry order ± multiplier*stop_size (BUY: +, SELL: -)
            tp_base_price = entry_price
            if buySellType == 'BUY':
            ...
            # Determine if extended hours / OTH: use stop-limit for entry; TP/SL sent after fill (not bracket)
            is_extended, session = _is_extended_outside_rth(outsideRth)
            use_stop_limit_entry = is_extended or outsideRth  # when True: entry is STP LMT, TP/SL sent after fill via sendTpAndSl
            ...
            # Place entry order first to get actual order ID (may change if duplicate detected)
            entry_response = connection.placeTrade(contract=contract, order=entry_order, outsideRth=outsideRth, trade_type=barType)
            # Get the actual entry order ID from response (may have been changed if duplicate was detected)
            actual_entry_order_id = entry_response.order.orderId
            logging.info(f"Conditional Order entry placed: orderId={actual_entry_order_id}, status={entry_response.orderStatus.status}")
            
            # For RTH: Place TP and SL orders as bracket (when entry was plain STP)
            if not use_stop_limit_entry:
                # Update bracket orders to use actual entry order ID (in case it was changed)
                if actual_entry_order_id != parent_order_id:
                    logging.info("Conditional Order RTH: Entry order ID changed from %s to %s, updating bracket orders", 
                               parent_order_id, actual_entry_order_id)
                    tp_order.parentId = actual_entry_order_id
                    sl_order.parentId = actual_entry_order_id
                
                tp_response = connection.placeTrade(contract=contract, order=tp_order, outsideRth=outsideRth, trade_type='TakeProfit')
                logging.info(f"Conditional Order TP placed: orderId={tp_response.order.orderId}, parentId={tp_response.order.parentId}")
                
                sl_response = connection.placeTrade(contract=contract, order=sl_order, outsideRth=outsideRth, trade_type='StopLoss')
                logging.info(f"Conditional Order SL placed: orderId={sl_response.order.orderId}, parentId={sl_response.order.parentId}")
            ...
            # Store stop/TP details for use in retries (Error 103) and TP/SL management
            entry_id_key = int(entry_response.order.orderId)
            if entry_id_key in Config.orderStatusData:
                order_state = Config.orderStatusData[entry_id_key]
                order_state['stopSize'] = stop_size
                # Legacy key (already used elsewhere)
                order_state['stopLossPrice'] = stop_loss_price
                # Standardized keys used by IBConnection.orderStatusEvent bracket regeneration
                order_state['stop_loss_price'] = stop_loss_price
                order_state['tp_price'] = tp_price
                # For Conditional Order in RTH we always use a simple stop order for SL
                order_state['stop_order_type'] = 'STP'
                # No separate stop_limit_price needed for RTH Conditional Order
                Config.orderStatusData[entry_id_key] = order_state
                logging.info(
                    "Stored Conditional Order TP/SL state in orderStatusData for %s: "
                    "stop_size=%s, stop_loss_price=%s, tp_price=%s, stop_order_type=%s",
                    entry_response.order.orderId,
                    stop_size,
                    stop_loss_price,
                    tp_price,
