    ...
    try:
        ...
        Config.orderStatusData.update({int(response.order.orderId):
                                           {'slValue':slValue, 'ordType': ordType, 'orderId': int(response.order.orderId),
                                            ...
                                            "risk": risk,"dateTime":datetime.datetime.now(),"entryData":entryData,"tif":tif,"barType":barType,"userBuySell":userBuySell,"userAtr":userAtr,"outsideRth":outsideRth,"replayEnabled":replayEnabled,"entry_points":entry_points}})
